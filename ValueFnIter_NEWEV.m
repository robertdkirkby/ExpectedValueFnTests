function [V,Policy,EVtime]=ValueFnIter_NEWEV(n_a,n_z,N_j,a_grid,z_gridvals_J,pi_z_J,ReturnFn,Parameters,DiscountFactorParamNames,vfoptions,timeEV)
...
%
% This is ValueFnIter_OLDEV with ONLY the EV-contraction block changed to the
% matmul pipeline with exact -Inf restoration (see EVtest.m):
%   EVinf=(EV==-Inf); EV(EVinf)=-1e250;
%   EV=EV*pi_z_J(:,:,jj)';
%   EV(EVinf*(pi_z_J(:,:,jj)'>0)>0)=-Inf;  % exact -Inf restoration
%   EV=reshape(EV,[N_a,1,N_z]);
% Everything else is byte-identical to ValueFnIter_OLDEV.
%
...
for reverse_j=1:N_j-1
    ...
    % --- EV contraction block (the NEW matmul pipeline with exact -Inf restoration) ---
    ...
    EV(EVinf*(pi_z_J(:,:,jj)'>0)>0)=-Inf; % exact -Inf restoration
    EV=reshape(EV,[N_a,1,N_z]); % leave a singular second dimension, as the broadcast's sum did
